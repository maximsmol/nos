...
main = shakeArgs shakeOptions' $ do
  ...
  path_krnl %> \out -> do
    ...
    need srcs
    () <- cmd "clang++" "-Wl,-T" [path_klink_script] "-nostdlib" "-o" [out] srcs  "-lgcc"
    return ()
  ...
  [path_kobj<//>"*"<.>"o", path_kdep<//>"*"<.>"dep"] &%> \[out, dep] -> do
    ...
    let crossFlags = ["-ffreestanding", "-m32"] --  "-mno-sse", "-nostdlib"
    let otherFlags = ["-std=c++1z", "-O0"]
    let command = ["clang++", "-fdiagnostics-color=always"] ++ warnFlags ++ crossFlags ++ otherFlags ++ ["-o", out]
